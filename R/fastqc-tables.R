...
#' 
#' @description \code{fastqc} function parses all the summary statistics 
#' produced by \code{FastQC} tool and returns a nested list data structure 
#' corresponding to each sample.
#' 
#' @param sample_info Full path to file containing details about 
#' \code{samples} and their \code{paths}. See \code{Details} section.
#' 
...
#' }
#' 
#' It can also optionally contain a \code{group} column. If present, then the 
#' plot generated with fastqc would be colored / facetted according to samples 
...
#' 
#' @description This is the workhorse that powers \code{fastqc}. Given a 
#' \code{sample} and \code{path}, \code{extract_summary_tables} extracts all 
...
extract_summary_tables <- function(doc, sample, pair, group) {
    ...
    extract <- function(i, j) {
        if (j-i-2L <= 0L) {
             message("Table at start index ", i, " seems to contain only ", 
                "one (header) row, therefore returning empty data.table.")
