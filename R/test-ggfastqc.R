...
#' 
#' @description Runs a set of tests to check if \code{ggfastqc} is working 
#' correctly.
#'
#' @details Runs a series of tests. These can be used to see features and 
#' examples of usage, too. Running \code{test_ggfastqc} will tell you the full 
#' location of the test file(s) to open.
#' @param verbose If \code{TRUE} sets \code{ggfastqc_verbose} to \code{TRUE} for 
...
#' otherwise.
#' @aliases test_ggfastqc
#' @seealso \code{\link{fastqc}}, \code{\link{plot_gc_stats}} 
#' \code{\link{plot_dup_stats}}
#' @examples
#' \dontrun{
#' ggfastqc:::test_ggfastqc()
#' }
test_ggfastqc <- function(verbose=FALSE, pkg="pkg", silent=FALSE) {
    if (exists("test_ggfastqc", .GlobalEnv, inherits=FALSE)) { # package developer
        if ("package:ggfastqc" %in% search()) stop("ggfastqc package loaded")
        if (.Platform$OS.type == "unix") {
            d = path.expand("~/Documents/oa-git/ggfastqc/inst/tests")
        }
    } else { # user
        d = paste(getNamespaceInfo("ggfastqc", "path"), "/tests", sep="")
    }
    olddir = setwd(d)
    ...
    for (fn in file.path(d, 'tests.Rraw')) {    # not testthat
        cat("Running", fn, "\n")
        oldverbose = getOption("ggfastqc_verbose")
        if (verbose) options(ggfastqc_verbose=TRUE)
        ...
        options(ggfastqc_verbose=oldverbose)
    }
    invisible(sum(sapply(envirs, `[[`, "nfail"))==0)
}

# Define test() and its globals here, for use in dev
# But primarily called by test_ggfastqc() calling inst/tests/tests.Rraw
...
test <- function(num, x, y, error=NULL, warning=NULL, output=NULL) {
    ...
    #    ggfastqc appropriate definition of "equals" different
    ...
    # to cater for both test_ggfastqc() and stepping through tests in dev
